...
describe('Ejercicio 6 - Puntos bi-dimensionales', () => {
  it('sumaPoints([6, 4], [3, 2]) returns value [9, 6]', () => {
    expect(sumaPoints([6, 4], [3, 2])).to.be.deep.equal([9, 6]);
  });

  it('restaPoints([6, 4], [3, 2]) returns value [3, 2]', () => {
    expect(restaPoints([6, 4], [3, 2])).to.be.deep.equal([3, 2]);
  });

  it('productoPoints([6, 4], 3) returns value [9, 6]', () => {
    expect(productoPoints([6, 4], 3)).to.be.deep.equal([18, 12]);
  });

  it('distEuclidea([6, 4], [3, 2]) returns value 3,61', () => {
    expect(distEuclidea([6, 4], [3, 2])).to.be.equal(3.61);
  });
});
